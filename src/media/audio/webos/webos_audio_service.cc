...
bool WebOSAudioService::GetDeviceList(
  ...
  std::unique_ptr<base::Value::Dict> root_value;
  ...
  for (auto& device_entry : devices->GetList()) {
    const base::Value::Dict* device = device_entry.GetIfDict();
    if (device) {
...
bool WebOSAudioService::GetRootDictionary(
    ...
    std::unique_ptr<base::Value::Dict>* root_dictionary) {
  ...
  if (root_dictionary)
    *root_dictionary =
        std::make_unique<base::Value::Dict>(std::move(root_value));
